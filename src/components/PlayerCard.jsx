...
export default function PlayerCard({ player }) {
    ...
    const handleDeductionClick = deduction => () => {
        mainManager.setters.togglePlayerDeduction(player.id, state.selectedLevelIndex, deduction)
    }
    ...
    return (
        <div className="p-2 bg-gray-300 rounded-md shadow-sm shadow-gray-800" style={{opacity: player.forfeit ? 0.7 : 1}}>
            <h1 className="relative bg-gray-200 p-1 mb-1 rounded-sm text-gray-800 shadow-inner shadow-gray-400 font-bold">
                {player.name.toUpperCase()}
                {player.forfeit && <FiFlag className="absolute right-2 top-2 animate-bounce cursor-pointer" onClick={() => mainManager.setters.togglePlayerForfeit(player.id)} />}
            </h1>
            <div className="grid grid-cols-12">
                ...
                <div className="col-span-3 grid grid-cols-2 gap-1">
                    ...
                    <button
                        ...
                        onClick={() => mainManager.setters.togglePlayerForfeit(player.id)}
                        title="Forfeit"
                    >
                        <FiFlag />
                    </button>
            ...
            >
                SCORE: {readableNum(Math.round(score))} 
            </div>
        </div>
    )
}